...
const SupportForm: FC = () => {
  const formik: FormikValues = useFormik({
    ...
    onSubmit: (value) => {
      const resultSubmit: ISubjectItem = {
        alias: value.service,
        ...
        inn: auth.identify ? auth.data.response.organizationINN : 0,
        text: value.text,
      };
      postSubject(resultSubmit).then((data) => {
        setPostHasSent(data);
      });
    },
  });
  ...
  const [postHasSent, setPostHasSent] = useState(undefined);
        ...
        <div className={Styles.support__form__item__answer}>
          ...
          {postHasSent && (
            <span>
              ...
              {postHasSent.response
                ? "Сообщение отправлено"
                : "Бэка енд забил на ошибки"}
            </span>
          )}
        </div>
      </form>
